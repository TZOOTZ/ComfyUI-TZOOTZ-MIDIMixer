...
"""
    ...
    def apply_ipadapter(self, model, ipadapter, image, weight, track_strength):
        """Apply IPAdapter with calculated weight"""
        if ipadapter is not None and image is not None and weight > 0 and track_strength > 0:
            final_weight = weight * track_strength
            # This would call the actual IPAdapter apply function
            # The exact implementation depends on the IPAdapter node being used
            tzootz_print(f"IPAdapter applied with weight: {final_weight:.2f}", 'INFO')
            # In real implementation:
            # return ipadapter.apply_ipadapter(model, image, final_weight)
        return model

    def apply_controlnet(self, positive, negative, control_net, image, strength, track_strength):
    ...
    def mix_with_midi(self, model, positive, negative, latent_image, 
            ...
            for i in range(4):
                if i < len(tracks):
                    ...
                    if ipa_key in kwargs and img_key in kwargs:
                        ...
                        model = self.apply_ipadapter(
                            model, kwargs[ipa_key], kwargs[img_key], 
                    ...
                    if cn_key in kwargs and cn_img_key in kwargs:
                        ...
                        positive, negative = self.apply_controlnet(
                            positive, negative,
        ...
        # Return mix values as tensor for further processing
        mix_values = torch.tensor(self.track_states)
        
        return (model, positive, negative, debug_info, mix_values)
